...
while True:
    try:
        ...
        data=json.loads((base/'upload.json').read_text())
        finished=base/'finished.json'
        if finished.exists():
            code=json.loads(finished.read_text())['exit_code']
            if code not in (0,2):
                data['complete']=False
                data['status']='Scan paused — partial results' if code==130 else 'Scan failed — partial results'
        ...
        if finished.exists():
            (base/'published-final.json').write_text(json.dumps({'updated_at':data['updated_at'],'complete':data['complete']}))
            break
